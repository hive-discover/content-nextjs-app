...
export default function PostResults(props){
    ...
    const searchBody = {query : search_query, amount : visiblePostsCount ? visiblePostsCount : amountPerPage, highlight : true, page_number : pageNumber};
    ...
    return (
        <Box sx={{width : "100%"}}>
...

            {
                ...
                ? <Link href={`/search/posts/${search_query}`}><Button variant="contained">View All Results</Button></Link> : null
            }
        </Box>
    )
}