...
export function chooseCommunityMode({name, title}, mode, session, isLoading, msg_encoded){
    ...
    switch(mode){
        ...
        case "trending":
            return {
                title : "Trending Posts",
                dataHook : () => useDiscussions({limit : 25, tag : name, sort : "trending", observer : username}),
                allowed : true
            };
        case "hot":
            return {
                title : "Hot Posts",
                dataHook : () => useDiscussions({limit : 25, tag : name, sort : "hot", observer : username}),
                allowed : true
            };
        case "new":
            return {
                title : "New Posts",
                dataHook : () => useDiscussions({limit : 25, tag : name, sort : "created", observer : username}),
                allowed : true
            }    
        case "muted":
            return {
                title : "Muted Posts",
                dataHook : () => useDiscussions({limit : 25, tag : name, sort : "muted", observer : username}),
                allowed : true
            }           
    }
}