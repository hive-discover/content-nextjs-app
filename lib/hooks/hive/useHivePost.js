...
export default function useHivePost(post){
    ...
    const {data : postData, error : postError} = useSWR({author : author.replace("@", ""), permlink}, getPost, {fallbackData : missingFields === 0 ? post : null});
    const isLoading = !postData && !postError;
    return {data : postData, pending : isLoading, error : postError};
}
